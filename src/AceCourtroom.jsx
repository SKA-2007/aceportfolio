...
const mapNodes = [
  {
    ...
    dialogueSprites: ["serious", "normal", "smiling"]
  },
  {
    id: "github",
    name: "GitHub & Git",
    ...
    dialogueSprites: ["serious", "normal"]
  },
  {
    id: "java",
    name: "Java",
    ...
    dialogueSprites: ["normal", "smiling"]
  },
  {
    id: "godot",
    ...
    dialogueSprites: ["smiling", "normal"]
  },
  {
    id: "python",
    name: "Python",
  ...
  },
  {
    ...
    dialogueSprites: ["serious", "smiling"]
  },
  {
    id: "react",
    name: "React",
...
// Testimony statements for Cross-Examination Mode with assigned witness sprite expressions per line
const testimonyStatements = [
  {
    ...
    witnessResponseSprites: ["mocking"],
    ...
    correctWitnessSprites: ["shocked", "frustrated"],
    ...
    wrongWitnessSprites: ["mocking", "smiling"],
  ...
  },
  {
    ...
    witnessResponseSprites: ["serious"],
    correctWitnessReaction: [
  ...
  evidence: [
    {
      ...
      dialogueSprites: ["smiling", "normal", "serious"]
    },
    {
      id: "floor_plan",
      title: "Skill Map",
    ...
    },
    {
      ...
      dialogueSprites: ["serious", "normal", "mocking"],
...
function AceAttorneyCourtRecord({ playSelect, sectionRef, onCheck, isInCourtRecord, setIsInCourtRecord, selectedItem, setSelectedItem }) {
  ...
  const handleScrollLeft = () => {
    ...
    if (scrollContainerRef.current) {
      ...
      if (children[nextIdx]) {
        children[nextIdx].scrollIntoView({ behavior: "smooth", inline: "center", block: "nearest" });
      }
    }
  };
  ...
  return (
    ...
    >
      <div className="w-full bg-slate-100 rounded-xl border-4 border-slate-300 shadow-[0_10px_30px_rgba(0,0,0,0.8)] overflow-hidden relative">
        <div className="w-full bg-slate-200 border-b-2 border-slate-300 flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-1 bg-cyan-600 text-white font-black text-base sm:text-lg px-5 py-1.5 rounded-sm shadow-sm tracking-widest uppercase [clip-path:polygon(0_0,90%_0,100%_100%,0%_100%)] pr-8">
            <span>{activeTab === "evidence" ? "Evidence" : "Profiles"}</span>
          </div>

          <button
            onClick={handleTabToggle}
            className="flex items-center gap-2 font-aceUi text-sm sm:text-base font-bold px-6 py-2.5 rounded border shadow-md transition-all bg-cyan-700 hover:bg-cyan-600 active:scale-95 text-white border-cyan-400 cursor-pointer"
          >
        ...
        <div className="p-6 sm:p-8 md:p-10 bg-white border-2 border-cyan-500 m-4 sm:m-5 rounded-md shadow-inner">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-6 items-stretch">
            <div className="bg-slate-200 border-2 border-slate-400 rounded-lg flex items-center justify-center p-6 h-64 sm:h-72 md:h-80 shadow-inner">
              <span className="select-none flex items-center justify-center">
                <RenderItemMedia item={selectedItem} sizeClass="w-48 h-48 sm:w-60 sm:h-60 md:w-64 md:h-64" />
              </span>
            </div>

            <div className="md:col-span-2 flex flex-col justify-start">
              <div className="bg-amber-300/90 border-b-2 border-amber-500 px-5 py-3 mb-4 rounded-sm shadow-sm">
                <h3 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-wide line-clamp-1">
                  {selectedItem?.title || "Empty File"}
                </h3>
              </div>

              <div className="space-y-4 font-aceUi text-slate-800 text-lg sm:text-xl md:text-2xl leading-relaxed">
                <div className="border-b border-dashed border-slate-300 pb-3 min-h-[90px] max-h-[90px] overflow-y-auto">
                  <p className="font-semibold">
                    {selectedItem?.description || "No inventory item recorded in this slot."}
                  </p>
                </div>
                <div className="border-b border-dashed border-slate-300 pb-3 min-h-[80px] max-h-[80px] overflow-y-auto">
                  {selectedItem?.details && (
                    <p className="text-slate-600 text-base sm:text-lg md:text-xl font-aceDialogue whitespace-pre-line tracking-wider">
                      {selectedItem.details}
                    </p>
                  )}
                </div>
          ...
          <div className="bg-cyan-900/90 p-4 rounded-lg border-2 border-cyan-600 shadow-inner flex items-center justify-center gap-4 relative">
            <button
              onClick={handleScrollLeft}
              className="p-1 hover:scale-110 active:scale-95 transition-transform cursor-pointer shrink-0 z-20"
              title="Previous Item"
            >
              <img src="/Sprites/select_arrow.png" alt="Previous" className="w-8 h-8 sm:w-10 sm:h-10 object-contain drop-shadow" />
            </button>

            <div 
              ref={scrollContainerRef}
              className="flex items-center gap-4 overflow-x-auto scrollbar-none py-2 px-2 scroll-smooth snap-x snap-mandatory max-w-[200px] sm:max-w-none justify-start sm:justify-center"
            >
              {currentItems.map((item, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`w-20 h-20 sm:w-24 sm:h-24 shrink-0 snap-center rounded border-2 relative flex items-center justify-center transition-all ${
                  ...
                  >
                    ...
                    <span className="select-none flex items-center justify-center">
                      <RenderItemMedia item={item} sizeClass="w-12 h-12 sm:w-16 sm:h-16" textFallback="" />
                    </span>
                  </div>
                );
              })}
            ...
            <button
              ...
              title="Next Item"
            >
              <img src="/Sprites/select_arrow.png" alt="Next" className="w-8 h-8 sm:w-10 sm:h-10 object-contain drop-shadow scale-x-[-1]" />
            </button>
          </div>

          <div className="flex items-center justify-end gap-4 mt-6 h-[50px]">
            {isInCourtRecord && activeTab === "evidence" && selectedIndex >= 2 && selectedIndex <= 5 && selectedItem && (
              <a
                ...
                className="flex items-center gap-2 font-aceUi text-base sm:text-lg font-bold px-7 py-2.5 rounded border shadow-md transition-all bg-emerald-700 hover:bg-emerald-600 active:scale-95 text-white border-emerald-400 cursor-pointer"
              >
                VIEW
              </a>
            )}
            ...
            <button
              ...
              className={`flex items-center gap-2 font-aceUi text-base sm:text-lg font-bold px-7 py-2.5 rounded border shadow-md transition-all ${
  ...
  return (
    ...
    >
      ...
      {isCrossExamActive && !isMapChecked && (
        <div className="w-full bg-slate-950/95 border-2 border-green-500/80 rounded-xl p-6 md:p-8 relative min-h-[300px] sm:min-h-[320px] shadow-[0_10px_35px_rgba(0,0,0,0.9)] flex flex-col justify-between backdrop-blur-md font-aceUi">
          <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-[90%] sm:w-auto text-center bg-green-600 border-2 border-green-300 text-white font-aceUi text-xs sm:text-sm md:text-base font-bold px-4 sm:px-6 py-1 shadow-md flex items-center justify-center [clip-path:polygon(5%_0%,_95%_0%,_100%_50%,_95%_100%,_5%_100%,_0%_50%)] z-40 uppercase tracking-wider">
            CROSS-EXAMINATION: WITNESS TESTIMONY
          </div>

          <div className="flex items-center justify-between border-b border-green-700/50 pb-2 mt-4">
          ...
          <div className="my-4 text-lg sm:text-xl md:text-2xl font-aceDialogue text-green-400 tracking-wider leading-relaxed min-h-[96px] sm:min-h-[108px] overflow-y-auto">
            <TypeIt
              key={statementIndex}
          ...
          <div className="w-full flex items-center justify-end border-t border-slate-700/50 pt-3 h-[52px]">
            <button
...
export default function AceCourtroom() {
  ...
  const [dialogueSpritesQueue, setDialogueSpritesQueue] = useState(["normal"]);
  ...
  // Preload all critical images when the portfolio is opened to ensure smooth rendering
  useEffect(() => {
    const imagesToPreload = [
      "/Sprites/character.png",
      "/Sprites/character_right.png",
      "/Sprites/Witness/frustrated.png",
      "/Sprites/Witness/mocking.png",
      "/Sprites/Witness/normal.png",
      "/Sprites/Witness/serious.png",
      "/Sprites/Witness/shocked.png",
      "/Sprites/Witness/smiling.png",
      "/Interjections/take_that.png",
      "/Interjections/hold_it.png",
      "/Interjections/objection.png",
      "/floor_plan.png",
      "/courtroom-bg.jpg",
      "/Sprites/search/blue.png",
      "/Sprites/search/blue_ticked.png",
      "/Sprites/search/yellow.png",
      "/Sprites/search/yellow_ticked.png"
    ];
  ...
  const triggerDialogue = (dialogueData, speaker = "Phoenix", activateCrossExamOnFinish = false, customWitnessReaction = null, customWitnessSprites = null) => {
    ...
    if (customWitnessReaction) {
      ...
      setPendingWitnessSprites(customWitnessSprites || Array(customWitnessReaction.length).fill("normal"));
    }

    if (speaker === "Xine Ohp" && customWitnessSprites) {
      setDialogueSpritesQueue(customWitnessSprites);
    } else if (speaker === "Xine Ohp") {
      setDialogueSpritesQueue(lines.map(() => "normal"));
    }
  ...
  // Determine current Xine Ohp sprite based on active speaker and current line index
  const getCurrentWitnessSprite = () => {
    if (activeSpeaker !== "Xine Ohp") return "normal";
    const spriteName = dialogueSpritesQueue[dialogueIndex] || "normal";
    return spriteName;
  };

  const currentWitnessSprite = getCurrentWitnessSprite();
  ...
  const handleHoldIt = (warningMessage, speakerName = "Phoenix", speakerSprites = null) => {
    setShowHoldIt(true);

    setTimeout(() => {
      setShowHoldIt(false);
      if (warningMessage) {
        triggerDialogue(warningMessage, speakerName, false, null, speakerSprites);
      }
    }, 900);
  };

  const [pendingWitnessResponse, setPendingWitnessResponse] = useState(null);
  const [pendingWitnessSprites, setPendingWitnessSprites] = useState(null);
  ...
  const triggerPressSequence = (statement) => {
    ...
    setTimeout(() => {
      ...
      const xineSprites = statement.witnessResponseSprites || Array(xineLines.length).fill("mocking");

      setPendingWitnessResponse(xineLines);
      setPendingWitnessSprites(xineSprites);
  ...
  const handleNextDialogueWithWitness = () => {
    if (dialogueIndex < dialogueQueue.length - 1) {
      ...
      const xineSprites = pendingWitnessSprites;
      setPendingWitnessResponse(null);
      setPendingWitnessSprites(null);
      triggerDialogue(xineLines, "Xine Ohp", false, null, xineSprites);
  ...
  const handlePresentEvidence = (statement) => {
    ...
    // Scroll to cross-examination section immediately upon clicking PRESENT or pressing R
    scrollToCrossExam();
  ...
  return (
    <div className="min-h-screen w-full bg-[#0b132b] text-white font-aceUi flex flex-col justify-between p-4 sm:p-6 md:p-8 select-none relative pb-52 sm:pb-60 overflow-x-hidden">
      {showTakeThat && (
      ...
      <main 
        id="home"
        className="flex-1 flex flex-col justify-between my-4 w-full max-w-6xl mx-auto rounded-xl border border-amber-500/20 relative overflow-hidden bg-cover bg-[32%_top] sm:bg-top bg-no-repeat min-h-[calc(100vh-180px)] p-4 md:p-6"
        ...
        <div className="relative z-10 w-full flex flex-row items-start justify-between gap-4">
          <div className="border-2 border-cyan-500/40 bg-slate-950/85 backdrop-blur-md p-2.5 rounded-lg shadow-2xl">
            ...
            <div className="flex flex-col gap-1 text-sm font-semibold text-slate-200">
              <a href="https://github.com" target="_blank" rel="noreferrer" className="flex items-center gap-1.5 p-1 rounded hover:bg-slate-800 hover:text-amber-400 transition-colors">
                <span>📦</span> Github
              </a>
      ...
      <div className={`fixed bottom-[110px] sm:bottom-[130px] md:bottom-[150px] left-1 sm:left-6 md:left-12 h-44 sm:h-60 md:h-80 lg:h-96 z-20 pointer-events-none flex items-end transition-opacity duration-300 ${showCharacter ? "opacity-100" : "opacity-0"}`}>
      ...
      <div className={`fixed bottom-[110px] sm:bottom-[130px] md:bottom-[150px] right-1 sm:right-6 md:right-12 h-44 sm:h-60 md:h-80 lg:h-96 z-20 pointer-events-none flex items-end transition-opacity duration-300 ${isCrossExamActive && isCrossExamInView ? "opacity-100" : "opacity-0"}`}>
        <img 
          key={currentWitnessSprite}
          src={`/Sprites/Witness/${currentWitnessSprite}.png`} 
          alt={`Xine Ohp - ${currentWitnessSprite}`} 
          onError={(e) => { e.target.src = "/Sprites/character_right.png"; }}
          className="h-full w-auto object-contain object-bottom drop-shadow-[0_12px_15px_rgba(0,0,0,0.8)] -scale-x-100 animate-[spriteTransition_0.25s_ease-out]"
        />
      </div>

      <div className="fixed bottom-0 left-0 right-0 w-full z-50 px-2 sm:px-4 md:px-6 pb-2 sm:pb-3">
        <div 
          onClick={() => {
            if (hasNextLine || pendingWitnessResponse || pendingCrossExamActivation || isMapDialogueActive) {
              handleNextDialogueWithWitness();
          ...
          className="w-full bg-slate-950/95 border-t-2 border-cyan-500/50 p-4 sm:p-6 md:p-8 relative min-h-[160px] sm:min-h-[180px] md:min-h-[190px] shadow-2xl flex flex-col justify-between backdrop-blur-md cursor-pointer select-none rounded-t-lg"
        >
          <div className={`absolute -top-3.5 sm:-top-4 left-4 sm:left-8 md:left-12 border-2 text-white font-aceUi text-xs sm:text-sm md:text-base font-bold px-5 py-0.5 shadow-md flex items-center justify-center [clip-path:polygon(10%_0%,_90%_0%,_100%_50%,_90%_100%,_10%_100%,_0%_50%)] z-40 ${
            activeSpeaker === "Xine Ohp" ? "bg-red-700 border-red-300" : "bg-cyan-600 border-cyan-300"
          }`}>
            {activeSpeaker}
          </div>

          <div className="mt-1 sm:mt-2 text-lg sm:text-2xl md:text-3xl font-aceDialogue text-slate-100 tracking-wider leading-relaxed min-h-[72px] sm:min-h-[84px] overflow-y-auto">
            <TypeIt
          ...
          <div className="w-full flex items-center justify-end gap-3 sm:gap-4 border-t border-slate-700/50 pt-2 mt-2 sm:mt-3 h-[44px]">
            {!isInCourtRecord && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="flex items-center gap-2 px-3 sm:px-4 py-1.5 bg-cyan-900/80 hover:bg-cyan-800 border border-cyan-500/50 rounded text-cyan-200 text-xs sm:text-sm font-bold tracking-wider transition-all shadow cursor-pointer active:scale-95"
              >
                <span className="hidden sm:inline-block bg-cyan-950 text-cyan-200 px-1.5 py-0.5 rounded text-xs border border-cyan-600 font-aceDialogue">
                  Q
                </span>
            ...
            {isCrossExamActive && (
              <button
                ...
                className="flex items-center gap-2 px-3 sm:px-4 py-1.5 bg-red-800/90 hover:bg-red-700 border border-red-500/70 rounded text-white text-xs sm:text-sm font-bold tracking-wider transition-all shadow cursor-pointer active:scale-95 animate-[fadeIn_0.2s_ease-in-out]"
              >
            ...
            {(hasNextLine || pendingWitnessResponse || pendingCrossExamActivation || isMapDialogueActive) && (
              <button
                ...
                title="Next Line"
              >
                <img src="/Sprites/select_arrow.png" alt="Next Arrow" className="w-7 h-7 sm:w-8 sm:h-8 object-contain drop-shadow scale-x-[-1] animate-pulse" />
              </button>
            )}
          </div>
        </div>
